...
rule Vidar_x64_v2_family {
	meta:
		...
		reference_3 = "8e04821fa87512991cb562a6aa4c3df695a63cb1261c5b5c7e9b9af37a3a50a9"
	strings:
		/*
		...
		/*
		 * Embedded-config repeating-XOR decoder. The low register bits in
		 * the ModR/M and SIB bytes are wildcarded because v2.2 preserves the
		 * loop but allocates its source/index registers differently
		 */
		$config_xor_loop = {
			4C 8B C9 4C 2B CA 4C 8B C2 49 F7 D8
			[0-96]
			4B 8D 04 ?? 48 F7 F7
			8A 0C 32 41 8A 0? 32 C8 43 88 0C 1?
			49 FF C? 4B 8D 04 ?? 49 3B C? 72 ??
		}
	condition:
		uint16(0) == 0x5A4D and
